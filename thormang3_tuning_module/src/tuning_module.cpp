...
void TuningModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
       ...
       it != robot->dxls_.end(); it++)
  {
    ...
    if(dxl_info->position_p_gain_item_ == 0)
      robot_tuning_data_[joint_name]->p_gain_ = NONE_GAIN;
    if(dxl_info->position_i_gain_item_ == 0)
      robot_tuning_data_[joint_name]->i_gain_ = NONE_GAIN;
    if(dxl_info->position_d_gain_item_ == 0)
      robot_tuning_data_[joint_name]->d_gain_ = NONE_GAIN;
    if(dxl_info->velocity_p_gain_item_ == 0)
      robot_tuning_data_[joint_name]->velocity_p_gain_ = NONE_GAIN;
    if(dxl_info->velocity_i_gain_item_ == 0)
      robot_tuning_data_[joint_name]->velocity_i_gain_ = NONE_GAIN;
    if(dxl_info->velocity_d_gain_item_ == 0)
      robot_tuning_data_[joint_name]->velocity_d_gain_ = NONE_GAIN;
...
void TuningModule::moveToTunePose(const std::string &pose_name)
{
  if(enable_ == false)
  {
    ROS_ERROR("thormang3_tuning_module is not enable!!");
    return;
  }
...
void TuningModule::queueThread()
{
  ...
  present_joint_pub_ = ros_node.advertise<thormang3_tuning_module_msgs::JointsOffsetPositionData>("/robotis/tuning_module/present_joints_data", 1);
                           ...
                           std::map<std::string, double> sensors)
{
       ...
       state_iter != result_.end(); state_iter++)
  {
    ...
    int v_p_gain = dxl->dxl_state_->velocity_p_gain_;
    int v_i_gain = dxl->dxl_state_->velocity_i_gain_;
    int v_d_gain = dxl->dxl_state_->velocity_d_gain_;
    ...
    joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]].velocity_p_gain_ = v_p_gain;
    joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]].velocity_i_gain_ = v_i_gain;
    joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]].velocity_d_gain_ = v_d_gain;
  }
  ...
  else if(get_tuning_data_ == true)
  {
    ...
    if(data_mutex_.try_lock() == true)
    {
      ...
      if (joint_name_to_id_it != joint_name_to_id_.end())
      {
        ...
        tuning_data_.velocity_p_gain_.getValue(joint_state_->goal_joint_state_[joint_id].velocity_p_gain_);
        tuning_data_.velocity_i_gain_.getValue(joint_state_->goal_joint_state_[joint_id].velocity_i_gain_);
        tuning_data_.velocity_d_gain_.getValue(joint_state_->goal_joint_state_[joint_id].velocity_d_gain_);
      }
       ...
       state_iter != result_.end(); state_iter++)
  {
    ...
    int v_p_gain = joint_state_->goal_joint_state_[joint_name_to_id_[joint_name]].velocity_p_gain_;
  ...
  if ((tuning_module_state_->cnt_ >= tuning_module_state_->all_time_steps_) && (tuning_module_state_->is_moving_ == true))
  {
    ...
    // update present joints state
    else
    {
      sendPresentJointOffsetData();
    }
  }
}
...
void TuningModule::jointOffsetDataCallback(const thormang3_tuning_module_msgs::JointOffsetData::ConstPtr &msg)
{
...
void TuningModule::jointGainDataCallback(const thormang3_tuning_module_msgs::JointOffsetData::ConstPtr &msg)
{
  if (this->enable_ == false)
  {
    ROS_ERROR("Tuning module is not enable");
  ...
  tuning_data_.velocity_p_gain_.setValue(msg->velocity_p_gain);
  tuning_data_.velocity_i_gain_.setValue(msg->velocity_i_gain);
  tuning_data_.velocity_d_gain_.setValue(msg->velocity_d_gain);
  ...
  robot_tuning_data_[msg->joint_name]->velocity_p_gain_ = msg->velocity_p_gain;
  robot_tuning_data_[msg->joint_name]->velocity_i_gain_ = msg->velocity_i_gain;
  robot_tuning_data_[msg->joint_name]->velocity_d_gain_ = msg->velocity_d_gain;
...
void TuningModule::jointTorqueOnOffCallback(const thormang3_tuning_module_msgs::JointTorqueOnOffArray::ConstPtr& msg)
{  
...
bool TuningModule::getPresentJointOffsetDataServiceCallback(
    thormang3_tuning_module_msgs::GetPresentJointOffsetData::Request &req,
    thormang3_tuning_module_msgs::GetPresentJointOffsetData::Response &res)
{
       ...
       map_it != robot_tuning_data_.end(); map_it++)
  {
    ...
    thormang3_tuning_module_msgs::JointOffsetPositionData joint_offset_pos;
    ...
    joint_offset_pos.velocity_p_gain = joint_data->velocity_p_gain_;
    joint_offset_pos.velocity_i_gain = joint_data->velocity_i_gain_;
    joint_offset_pos.velocity_d_gain = joint_data->velocity_d_gain_;
...
void TuningModule::sendPresentJointOffsetData()
{
  thormang3_tuning_module_msgs::JointsOffsetPositionData present_data;
       ...
       map_it != robot_tuning_data_.end(); map_it++)
  {
    ...
    present_data.present_data_array.push_back(joint_offset_pos);
    std::cout << "joint : " << joint_name << ", p_gain : " << joint_data->p_gain_ << std::endl;
  }

  present_joint_pub_.publish(present_data);
}
